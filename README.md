...
### MUSS(EN) - Estrutura de arquivos criada

Primeiramente vamos conhecer os arquivos criados nesse processo:

* **mpes-métodos/muss/scripts/ementas/punkt_sentence_token.py:** Classe criada para a indentificação das sentenças em uma ementa, bem como tratamento das abraviações comuns tanto na língua portuguesa quanto no meio jurídico;
* **mpes-métodos/muss/scripts/ementas/dataset:** Diretório que contém os datasets do TRF5 e STF com as ementas usadas no processo de simplificação.

Dentro do diretório "exp01" criado para o método MUSS(EN), temos os seguintes arquivos:
...
```
cd muss/  
source venv_python/bin/activate  # no diretório do MUSS ativar o ambiente virtual Python configurado para o método.
...
* **punkt_sentence_token.py**: Classe de identificação de sentenças já usada nos métodos anteriores;
...
```
cd transformers/ 
source venv_python/bin/activate  # Após ter criado o ambiente virtual python desse método.
...
Em `mpes-métodos/pylinguistics/pylinguistics` estão os scripts criados para o processo de cálculo das métricas de legibilidade usadas para avaliar os resulados obtidos pelos métodos no processo de simplificação das ementas. Os arquivos foram:

* **metrica_ementa.py:** Classe criada para calcular as métricas e gerar as planilhas com os resultados
* **run_metrica_muss_en.py:** Gera as métricas do método MUSS(EN);
* **run_metrica_muss_pt.py:** Gera as métricas do método MUSS(PT);
* **run_metrica_transformers.py:** Gera as métricas do método Transformers;
* **run_metrica_nmt_attention.py:** Gera as métricas do método NMT + Attention.

Os resultados obtidos para os métodos foram os seguintes:

![Modelo gerado pelo processo de treinamento](assets/images/resultados_pylinguistics.png)

## Citação

Se você usou os artefatos gerados nessa pesquisa, por favor cite [Simplificação automática de textos jurídicos em português usando aprendizagem de máquina](http://link_artigo_ou_dissertacao)
